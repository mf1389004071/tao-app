		...
		<scroll-view v-if="true" scroll-y class="data_body" :style="{ height: scrollHeight }">
			...
			<view class="friend_operate">
				...
				<view class="charts-box">
					<qiun-data-charts type="mix" :chartData="userARPU" :reshow="delayload" :canvas2d="isCanvas2d"
						canvasId="two_a"
			...
			<view class="friend_operate">
				...
				<senior-table :headers="dataTable.headers" :contents="dataTable.contents" :urlCol="dataTable.urlCol"
					:firstLineFixed="true" :sortCol="dataTable.sortCol" :computedCol="dataTable.computedCol"
					:formatCol="dataTable.formatCol"></senior-table>
			</view>
			<view class="split_line"></view>
			...
			<view class="friend_operate">
				...
				<view style="display: flex;justify-content: space-between;">
					<view v-for="(item, index) in xProductDropPrecent" :key="index" class="charts-box"
						style="height: 130px;width: 45%;">
						<qiun-data-charts type="arcbar" :chartData="item" :canvasId="'two_c_' + index" :canvas2d="true"
							:reshow="delayload"
			...
			<view class="friend_operate">
				...
				<view class="charts-box">
					<qiun-data-charts type="mix" :chartData="illnessDropPrecent" :reshow="delayload" :canvas2d="true"
						canvasId="two_d" :opts="{ yAxis: { data: [{ title: '' }] } }" />
				</view>
			</view>
			...
			<view class="friend_operate">
				...
				<view style="display: flex;justify-content: space-between;">
					<view v-for="(item, index) in wProductDropPrecent" :key="index" class="charts-box"
						style="height: 130px;width: 45%;">
						<qiun-data-charts type="arcbar" :chartData="item" :canvasId="'two_e_' + index" :canvas2d="true"
							:reshow="delayload"
							:opts="{ title: { name: item.series[0].data * 100 + '%', color: item.series[0].color, fontSize: 25 }, subtitle: { name: item.series[0].name, color: '#666666', fontSize: 12 } }" />
					</view>
				</view>
...
export default {
	...
	components: {
		DataProgress, SeniorTable, TextBlock
	},
	data() {
		return {
			userOperateLineBar,
